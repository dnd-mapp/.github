...
function deriveReleaseBranchName(newVersion2) {
  const parsed = (0, import_semver.parse)(newVersion2);
  if (!parsed) {
    throw new Error(`Invalid version: "${newVersion2}"`);
  }
...
async function writePackageVersion(params) {
  ...
  manifest2["version"] = params.newVersion;
  await writeFile(params.manifestPath, JSON.stringify(manifest2, null, 2) + "\n", { encoding: "utf-8" });
}

// src/scripts/bump-version.mts
import { appendFile, readFile as readFile2 } from "fs/promises";
var versionInput = process.env["VERSION"];
var prereleaseIdInput = process.env["PRERELEASE_ID"];
var manifestPath = `${process.env["GITHUB_WORKSPACE"]}/package.json`;
var manifest = JSON.parse(await readFile2(manifestPath, "utf-8"));
var preid = prereleaseIdInput !== "none" ? prereleaseIdInput : void 0;
var newVersion = bumpVersion({ currentVersion: manifest.version, bumpType: versionInput, preid });
